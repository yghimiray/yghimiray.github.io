 ...
 */
quiz.scoreStudent = function (stuId) {
const foundStud = this.students.find(item => item.sid === stuId);
foundStud.answers.sort(answerComparator);
let score = 0;
...
for (const element of this.students){
    let stuScore = this.scoreStudent(element.sid);
    sum += stuScore;
}
